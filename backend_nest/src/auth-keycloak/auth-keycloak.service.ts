...
export class AuthKeycloakService {
  ...
  /**
   * B1.1 Đăng nhập trực tiếp bằng Username & Password (Direct Access Grant)
   */
  async loginDirectGrant(username: string, password: string): Promise<any> {
    try {
      const config = await this.getEffectiveConfig();
      ...
      const accessToken = res.data.access_token;
      if (!accessToken) {
        throw new HttpException('Không lấy được access token', HttpStatus.UNAUTHORIZED);
      }
      ...
      this.logger.warn(`Direct grant login failed for ${username}: ${err.response?.data?.error_description || err.message}`);
      throw new UnauthorizedException(
        err.response?.data?.error_description || 'Tài khoản hoặc mật khẩu không chính xác'
      );
    }
  }
